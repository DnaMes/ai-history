...
def test_delete_remembers_tombstone_and_redirects_back(monkeypatch, tmp_path):
    ...
    index_path.write_text(
        json.dumps(
            {
                'stats': {'total_sessions': 1, 'total_messages': 1, 'by_tool': {'warp': 1}, 'by_project': {}},
                'sessions': [
                    {
                        'id': 'ses-del-1',
                        'tool': 'warp',
...
def test_load_index_filters_tombstoned_sessions(monkeypatch, tmp_path):
    ...
    index_path.write_text(
        json.dumps(
            {
                'stats': {'total_sessions': 2, 'total_messages': 2, 'by_tool': {'warp': 2}, 'by_project': {}},
                'sessions': [
                    {'id': 'ses-keep', 'tool': 'warp', 'title': 'Keep', 'messages': 1, 'prompts': 1},
                    {'id': 'ses-delete', 'tool': 'warp', 'title': 'Delete', 'messages': 1, 'prompts': 1},
                ],
            }
        ),
        encoding='utf-8',
