...
def _scan_depth(default: int) -> int:
    raw = os.environ.get("AI_HISTORY_HOME_SCAN_DEPTH", str(default)).strip()
    try:
...
def _scan_limit(default: int) -> int:
    raw = os.environ.get("AI_HISTORY_HOME_SCAN_LIMIT", str(default)).strip()
    try:
        return max(1, int(raw))
    except ValueError:
        return default
...
def _scan_node_limit(default: int) -> int:
    raw = os.environ.get("AI_HISTORY_HOME_SCAN_NODE_LIMIT", str(default)).strip()
    try:
...
def _scan_time_budget_seconds(default: float) -> float:
    raw = os.environ.get("AI_HISTORY_HOME_SCAN_TIME_BUDGET_SECONDS", str(default)).strip()
    try:
